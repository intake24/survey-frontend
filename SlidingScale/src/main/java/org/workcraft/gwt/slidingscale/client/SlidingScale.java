...
public class SlidingScale extends Composite {
  ...
  public SlidingScale(final SlidingScaleDef definition, double limit, double initialLevel, final Function1<Double, String> labelfunc) {
     ...
     * 
     * @Override public void onMouseMove(MouseMoveEvent event) { int mouseX =
     * event.getRelativeX(baseImage.getElement()); int mouseY =
     * event.getRelativeY(baseImage.getElement());
     * 
     * int mouseOverArea = -1;
     * 
     * for (int i = 0; i < definition.areas.length; i++) { if
     * (definition.areas[i].shape.isInside(new Point(mouseX, mouseY))) {
     * mouseOverArea = i; break; } }
     * 
     * if (mouseOverArea != activeArea) { if (hasFocus && mouseOverArea == -1) {
     * setActiveArea(lastActiveArea); } else setActiveArea(mouseOverArea); } }
     * };
     * 
     ...
     * 
     * if (event.getNativeEvent().getKeyCode() == KeyCodes.KEY_LEFT) { prev();
     * event.preventDefault(); } if (event.getNativeEvent().getKeyCode() ==
     * KeyCodes.KEY_RIGHT) { next(); event.preventDefault(); } if
     * (event.getNativeEvent().getKeyCode() == KeyCodes.KEY_UP) { next();
     * event.preventDefault(); } if (event.getNativeEvent().getKeyCode() ==
     * KeyCodes.KEY_DOWN) { prev(); event.preventDefault(); } } },
     * KeyDownEvent.getType());
     * 
     * addDomHandler(new FocusHandler() {
     * 
     * @Override public void onFocus(FocusEvent event) { hasFocus = true; if
     * (activeArea == -1) setActiveArea(0); } }, FocusEvent.getType());
     * 
     * addDomHandler(new BlurHandler() {
     * 
     * @Override public void onBlur(BlurEvent event) { hasFocus = false;
     * setActiveArea(-1); } }, BlurEvent.getType());
     * 
     * initWidget(imageDiv);
     * 
     * prefetchImages();
