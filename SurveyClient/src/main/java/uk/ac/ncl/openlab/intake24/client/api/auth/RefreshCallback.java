...
public class RefreshCallback implements RequestCallback {
    ...
    private void authenticateWithToken(String authToken) {
        AuthenticationService.INSTANCE.signinWithToken(authToken, new MethodCallback<SigninResult>() {
            ...
            public void onFailure(Method signinMethod, Throwable exception) {
                if (signinMethod.getResponse().getStatusCode() == Response.SC_UNAUTHORIZED) {
                    ...
                    EmbeddedData.originatingUrl.accept(new Option.SideEffectVisitor<String>() {
                        @Override
            ...
            public void onSuccess(Method signinMethod, SigninResult response) {
                ...
                String urlWithoutToken = Window.Location.createUrlBuilder()
                        .removeParameter(UrlParameterConstants.authTokenKey)
                        .removeParameter(UrlParameterConstants.createUserKey)
                        .buildString();
    ...
    public void onResponseReceived(final Request request, Response response) {
        ...
        if (code == Response.SC_UNAUTHORIZED) {
            ...
            } else if (Window.Location.getParameter(UrlParameterConstants.createUserKey) != null) {

                String encodedParameters = Window.Location.getParameter(UrlParameterConstants.createUserKey);

                GeneratedUsersService.INSTANCE.createUser(EmbeddedData.surveyId, encodedParameters, new MethodCallback<CreateUserResponse>() {
                    @Override
                    public void onFailure(Method method, Throwable exception) {
                        genUserUI.show();

                        if (method.getResponse().getStatusCode() == 403 || method.getResponse().getStatusCode() == 400) {
                            genUserUI.onForbidden();
                        } else {
                            genUserUI.onServiceError();
                        }
                    ...
                    @Override
                    public void onSuccess(Method method, CreateUserResponse response) {
                        DynamicRedirect.set(response.userId, response.redirect);
                        authenticateWithToken(response.authToken);
                    }
                });
