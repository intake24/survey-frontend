...
public class MealPanel extends Composite {
    ...
    private void addFoodRow(UnorderedList<FlowPanel> list, final WithIndex<FoodEntry> f) {
        ...
        boolean thisFoodSelected = selection.accept(new Selection.Visitor<Boolean>() {
            @Override
            public Boolean visitMeal(Selection.SelectedMeal meal) {
                return false;
            }

            @Override
            public Boolean visitFood(Selection.SelectedFood food) {
                return (food.mealIndex == mealIndex) && (food.foodIndex == f.index);
            }

            @Override
            public Boolean visitNothing(Selection.EmptySelection selection) {
                return false;
            }
        });
    ...
    public MealPanel(final Meal meal, final int mealIndex, final Selection selection, final Callback1<Selection> requestSelectionChange) {
        ...
        boolean thisMealSelected = selection.accept(new Selection.Visitor<Boolean>() {
            @Override
            public Boolean visitMeal(Selection.SelectedMeal meal) {
