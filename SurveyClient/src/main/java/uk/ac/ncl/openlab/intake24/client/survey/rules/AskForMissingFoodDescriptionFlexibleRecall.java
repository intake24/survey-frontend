...
public class AskForMissingFoodDescriptionFlexibleRecall implements PromptRule<FoodEntry, FoodOperation> {
    ...
    public Option<Prompt<FoodEntry, FoodOperation>> apply(FoodEntry data, SelectionMode selectionType, PSet<String> surveyFlags) {
        if (data.isMissing() && data.flags.contains(MissingFood.NOT_HOME_RECIPE_FLAG) && !((MissingFood) data).isDescriptionComplete())
    ...
    public static WithPriority<PromptRule<FoodEntry, FoodOperation>> withPriority(int priority) {
        return new WithPriority<PromptRule<FoodEntry, FoodOperation>>(new AskForMissingFoodDescriptionFlexibleRecall(), priority);
    }
}