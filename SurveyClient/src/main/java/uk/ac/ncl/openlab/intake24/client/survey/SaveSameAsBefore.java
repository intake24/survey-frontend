...
import uk.ac.ncl.openlab.intake24.client.survey.rules.ShowAssociatedFoodPrompt;
                            ...
                            public void visitSome(Pair<Meal, FoodEntry> oldState) {
                                if (oldState.right.isEncoded()) {
                                    ...
                                    if (!portionSizeComplete(oldState.left, encOld) && portionSizeComplete(m, encNew)) {
                                        StateManagerUtil.saveSameAsBefore(AuthCache.getCurrentUserName(), m, encNew, scheme_id, version_id);
                                    }
                                }
                            }
