        ...
        if (!survey.customData.containsKey(PROXY_KEY) && survey.portionSizeComplete()) {
            ...
            SafeHtml promptText = SafeHtmlUtils.fromSafeConstant(
                    "<p>Did you complete this recall on behalf of someone else?</p>" +
                            "<p>Please note that recalls should be completed by the individual named on the first screen " +
                            "and only completed by someone else when necessary i.e. for young children, if there are " +
                            "language difficulties or the person is not confident in using a computer to fill in " +
                            "the recall.</p>"
            );
