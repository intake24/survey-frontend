...
public class TemplateFood extends FoodEntry {
    ...
    private boolean isComponentComplete(int index) {
        TemplateFoodData.ComponentDef def = data.template.get(index);

        int count = components.get(index).size();

        if (def.type == TemplateFoodData.ComponentType.Optional && markedAsComplete.contains(index)) {
            return true;
        } else {
            if (def.occurence == TemplateFoodData.ComponentOccurence.Single)
                return count == 1;
            else
