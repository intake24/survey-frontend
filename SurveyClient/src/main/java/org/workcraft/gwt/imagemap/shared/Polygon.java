...
public class Polygon {
  ...
  private void verifyLength() {
    if (this.vertices.length < 3)
      throw new IllegalArgumentException("Too few distinct vertices for polygon definition");
  }
  ...
  public Polygon(double[] coords) {
    if (coords.length < 6)
      throw new IllegalArgumentException("Too few coordinates for polygon definition");
