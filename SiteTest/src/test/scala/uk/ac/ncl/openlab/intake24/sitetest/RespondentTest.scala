...
import org.scalatest.{BeforeAndAfterAll, CancelAfterFailure, FunSuite}
...
class RespondentTest extends FunSuite with BeforeAndAfterAll with CancelAfterFailure with WebBrowser {

  if (System.getProperty("webdriver.chrome.driver") == null)
    throw new RuntimeException("Please set the webdriver.chrome.driver system property to point at the Selenium chromedriver executable")

  val host = System.getProperty("intake24.host")

  if (host == null)
    throw new RuntimeException("Please set the intake24.host property to point at the address you want to test")
  ...
  def waitForElement(q: String) = new WebDriverWait(webDriver, 2).until(ExpectedConditions.elementToBeClickable(By.cssSelector(q)))
  ...
  def clickButtonById(id: String) = {
    waitForElement("#"+id)

    actions.moveToElement(webDriver.findElement(By.id(id))).click().perform()
  }
  ...
  test("Proceed from welcome page") {
    click on waitForElement("#welcomePageReadyButton")
  }
  ...
  test("Butter confirm number of slices") {
    nextPrompt
  }
  ...
  test("Don't separate salad") {
    clickButtonByLabelAndWait("Keep as single food")
  }
  ...
  test("Report missing food / home recipe") {
    clickButtonById("intake24-food-browser-cant-find-button")
